...
class _ToyModel(torch.nn.Module):
    def __init__(self, vocab_size: int = 16, seq_len: int = 6) -> None:
        super().__init__()
    ...
    def __init__(self) -> None:
        ...
        self.samples = [
            ...
            },
            {
                ...
                "sample_id": "math-1",
                "teacher_logits": torch.randn(seq, vocab),
            },
            {
                "input_ids": torch.arange(seq, dtype=torch.long) + 4,
                "attention_mask": torch.ones(seq, dtype=torch.long),
                "sample_type": "code",
                "sample_id": "code-1",
                "teacher_logits": torch.randn(seq, vocab),
...
def test_hybrid_router_invokes_lm_teacher_only_for_lm(tmp_path):
    dataset = _HybridDataset()
    loader = DataLoader(dataset, batch_size=3, collate_fn=data.collate_batch)
    model = _ToyModel()
    teacher = _Teacher()
    trainer = Trainer(
        model=model,
    ...
    called_batch = teacher.calls[0]
    assert called_batch.shape[0] == 1  # only LM example triggers online KD
    assert torch.equal(called_batch.squeeze(0), torch.arange(model.seq_len, dtype=torch.long))
